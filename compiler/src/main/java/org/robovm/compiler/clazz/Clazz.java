...
public abstract class Clazz implements Comparable<Clazz> {
	...
	Clazz(Clazzes clazzes, String fileName, AbstractPath path) {
		...
		// Carl fix : https://groups.google.com/forum/#!topic/robovm/QUQpJBOdLBc
		String className = fileName.replace(File.separatorChar, '.').substring(
				0, fileName.lastIndexOf(".class"));
		className = fileName.replace('/', '.').substring(0,
