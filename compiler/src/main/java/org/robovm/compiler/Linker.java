...
public class Linker {
	...
	public void link(Set<Clazz> classes) throws IOException {
		...
		mb.addInclude(getClass().getClassLoader().getResource(
				String.format("header-%s-%s.ll", config.getOs().getFamily(),
						config.getArch())));
		mb.addInclude(getClass().getClassLoader().getResource("header.ll"));

		mb.addGlobal(new Global("_bcDynamicJNI", new IntegerConstant(config
				.isUseDynamicJni() ? (byte) 1 : (byte) 0)));
		...
		for (Clazz clazz : linkClasses) {
			...
			StructureConstant infoErrorStruct = createClassInfoErrorStruct(mb,
					clazz.getClazzInfo());
			Global info = null;
			if (infoErrorStruct == null) {
				info = new Global(mangleClass(clazz.getInternalName())
						+ "_info_struct", external, I8_PTR, false);
			} else {
				typeInfo.error = true;
				info = new Global(mangleClass(clazz.getInternalName())
						+ "_info_struct", infoErrorStruct);
			}
			mb.addGlobal(info);
			if (clazz.isInBootClasspath()) {
				bcpHashGen.put(clazz.getInternalName(), new ConstantBitcast(
						info.ref(), I8_PTR));
			} else {
				cpHashGen.put(clazz.getInternalName(),
						new ConstantBitcast(info.ref(), I8_PTR));
			}
		}
		// ArrayConstantBuilder test = new ArrayConstantBuilder(I8_PTR);
		// test.add(new NullConstant(Type.I8_PTR));
		ConstantGetelementptr l1 = new ConstantGetelementptr(mb.newGlobal(
				bcpHashGen.generate(), true).ref(), 0, 0);
		// ConstantGetelementptr l1 = new ConstantGetelementptr(mb.newGlobal(
		// test.build()).ref(), 0, 0);
		mb.addGlobal(new Global("_bcBootClassesHash", l1));

		ConstantGetelementptr l2 = new ConstantGetelementptr(mb.newGlobal(
				cpHashGen.generate(), true).ref(), 0, 0);
		mb.addGlobal(new Global("_bcClassesHash", l2));

		// mb.addGlobal(new Global("__bcBootClassesHash", l1));
		// mb.addGlobal(new Global("__bcClassesHash", l2));

		// mb.addGlobal(new Global("bcBootClassesHash", l1));
		// mb.addGlobal(new Global("bcClassesHash", l2));
		...
		for (Path path : allPaths) {
			...
			if (config.isSkipInstall() && config.getTarget().canLaunchInPlace()) {
				...
				entryName = config.getTarget().getInstallRelativeArchivePath(
						path);
			}
		...
		mb.addGlobal(new Global("_bcBootclasspath", new ConstantGetelementptr(
				mb.newGlobal(bootClasspathValues.build()).ref(), 0, 0)));
		mb.addGlobal(new Global("_bcClasspath", new ConstantGetelementptr(mb
				.newGlobal(classpathValues.build()).ref(), 0, 0)));

		if (config.getMainClass() != null) {
			mb.addGlobal(new Global("_bcMainClass", mb.getString(config
					.getMainClass())));
		}
		...
		for (Clazz clazz : linkClasses) {
			...
			if (typeInfo.error) {
				...
				mb.addGlobal(new Global(mangleClass(clazz.getInternalName())
						+ "_typeinfo", new StructureConstantBuilder()
						.add(new IntegerConstant(typeInfo.id))
						.add(new IntegerConstant(0))
						.add(new IntegerConstant(-1))
						.add(new IntegerConstant(0))
						.add(new IntegerConstant(0)).build()));
			} else {
				...
				mb.addGlobal(new Global(
						mangleClass(clazz.getInternalName()) + "_typeinfo",
						new StructureConstantBuilder()
								.add(new IntegerConstant(typeInfo.id))
								.add(new IntegerConstant(
										(typeInfo.classTypes.length - 1) * 4 + 5 * 4))
								.add(new IntegerConstant(-1))
								.add(new IntegerConstant(
										typeInfo.classTypes.length))
								.add(new IntegerConstant(
										typeInfo.interfaceTypes.length))
								.add(new ArrayConstantBuilder(I32)
										.add(classIds).build())
								.add(new ArrayConstantBuilder(I32).add(
										interfaceIds).build()).build()));

				if (!ci.isInterface() && !ci.isFinal()
						&& typeInfo.children.isEmpty()) {
					...
					for (MethodInfo mi : ci.getMethods()) {
						...
						if (!name.equals("<clinit>") && !name.equals("<init>")
								&& !mi.isPrivate() && !mi.isStatic()
								&& !mi.isFinal() && !mi.isAbstract()) {
		...
		Module module = Module.parseIR(context, mb.build().toString(),
				"linker.ll");
	...
	private TypeInfo buildTypeInfo(TypeInfo typeInfo,
			Map<ClazzInfo, TypeInfo> typeInfos) {
		...
		if (!ci.isInterface()) {
			if (ci.hasSuperclass()) {
				TypeInfo superTypeInfo = buildTypeInfo(
						typeInfos.get(ci.getSuperclass()), typeInfos);
		...
		if (!clTypeInfos.isEmpty()) {
			typeInfo.classTypes = clTypeInfos.toArray(new TypeInfo[clTypeInfos
					.size()]);
		}
		if (!ifTypeInfos.isEmpty()) {
			typeInfo.interfaceTypes = ifTypeInfos
					.toArray(new TypeInfo[ifTypeInfos.size()]);
		}
	...
	private StructureConstant createClassInfoErrorStruct(ModuleBuilder mb,
			ClazzInfo ci) {
		/*
		...
		if (!ci.isInterface() && ci.hasSuperclass()) {
			...
			if (superclazz.isPhantom()) {
				...
				errorMessage = String.format(
						"class %s has interface %s as super class", ci,
						superclazz);
			}
		...
		if (errorType == ClassCompiler.CI_ERROR_TYPE_NONE) {
			...
			for (ClazzInfo interfaze : ci.getInterfaces()) {
				if (interfaze.isPhantom()) {
					...
					errorType = ClassCompiler.CI_ERROR_TYPE_INCOMPATIBLE_CLASS_CHANGE;
					errorMessage = String
							.format("class %s tries to implement class %s as interface",
									ci, interfaze);
					break;
				}
			}
	...
	private Function createCheckcast(ModuleBuilder mb, Clazz clazz,
			TypeInfo typeInfo) {
		...
		if (typeInfo.error) {
			...
			Value result = call(fn, CHECKCAST_CLASS, fn.getParameterRef(0),
					info, fn.getParameterRef(1), new IntegerConstant(
							(typeInfo.classTypes.length - 1) * 4 + 5 * 4),
					new IntegerConstant(typeInfo.id));
			fn.add(new Ret(result));
		} else {
			Value result = call(fn, CHECKCAST_INTERFACE, fn.getParameterRef(0),
					info, fn.getParameterRef(1), new IntegerConstant(
							typeInfo.id));
			fn.add(new Ret(result));
		}
	...
	private Function createInstanceof(ModuleBuilder mb, Clazz clazz,
			TypeInfo typeInfo) {
		...
		if (typeInfo.error) {
			...
			Value result = call(fn, INSTANCEOF_CLASS, fn.getParameterRef(0),
					info, fn.getParameterRef(1), new IntegerConstant(
							(typeInfo.classTypes.length - 1) * 4 + 5 * 4),
					new IntegerConstant(typeInfo.id));
			fn.add(new Ret(result));
		} else {
			Value result = call(fn, INSTANCEOF_INTERFACE,
					fn.getParameterRef(0), info, fn.getParameterRef(1),
					new IntegerConstant(typeInfo.id));
			fn.add(new Ret(result));
		}
		return fn;
	...
	private Function createLookup(ModuleBuilder mb, ClazzInfo ci, MethodInfo mi) {
		...
		String targetFnName = mangleMethod(ci.getInternalName(), mi.getName(),
				mi.getDesc());
	...
	private Value getInfoStruct(ModuleBuilder mb, Function f, Clazz clazz) {
		return new ConstantBitcast(mb.getGlobalRef(mangleClass(clazz
				.getInternalName()) + "_info_struct"), I8_PTR_PTR);
	}
